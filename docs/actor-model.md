...
| X3 | **Archival & backup** configuration | ◐ (MVP-12) — `docs/ops-runbook.md` documents the scheduled `pg_dump` → S3 lifecycle (STANDARD_IA → Glacier → expire) + a quarterly restore drill + PITR notes. Still **documentation, not automation** — the schedule/lifecycle are deploy-environment concerns. ADR-039 | ops |
