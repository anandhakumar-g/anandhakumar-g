...
import { catalog, kyc as kycApi, superProviders } from "@/api/endpoints";
import { groupVendorCategories, KycDocView, SuperProviderView } from "@/api/types";
...
export default function SuperProvidersScreen() {
  const { theme } = useTheme();
  const list = useAsync(() => superProviders.list(), []);
  ...
  async function create() {
    ...
    setErr(null);
    try {
      await superProviders.create({ name: name.trim(), vendorCategoryId: vcat, company: true, contactPhone: phone.trim() });
      setName("");
      setPhone("");
      setVcat(null);
      setAdding(false);
  ...
  return (
    <Screen onRefresh={list.refresh} refreshing={list.refreshing}>
      ...
      <AppText tone="muted" style={{ marginTop: -theme.space(1) }}>
        Onboard, verify and tier providers. Communities enrol from this verified list.
      </AppText>
      ...
      {adding ? (
        <Card style={{ gap: theme.space(2) }}>
          ...
          <AppText size="sm" weight="600" tone="muted">Category</AppText>
      ...
      {list.loading ? (
        ...
        <EmptyState title="No providers yet" body="Add a vendor, review their KYC, then verify them." />
      ) : (
        list.data!.map((p: SuperProviderView) => (
          <Card key={p.id} style={{ gap: theme.space(2) }}>
            ...
            <AppText tone="primary" size="sm" onPress={() => setOpenKyc((cur) => (cur === p.id ? null : p.id))}>
            ...
            <View style={{ flexDirection: "row", gap: theme.space(2), flexWrap: "wrap" }}>
              {p.verificationStatus !== "VERIFIED" ? (
                <Button label="Verify" fullWidth={false} loading={busyId === p.id}
                  onPress={() => act(p.id, () => superProviders.verify(p.id, "VERIFIED"))} />
              ) : (
                <Button label="Suspend" variant="secondary" fullWidth={false} loading={busyId === p.id}
                  onPress={() => act(p.id, () => superProviders.verify(p.id, "SUSPENDED"))} />
              )}
              {p.verificationStatus !== "REJECTED" ? (
                <Button label="Reject" variant="danger" fullWidth={false} loading={busyId === p.id}
                  onPress={() => act(p.id, () => superProviders.verify(p.id, "REJECTED"))} />
              ) : null}
              <Button
                label={p.tier === "FEATURED" ? "★ Unfeature" : "Make featured"}
                variant={p.tier === "FEATURED" ? "primary" : "secondary"}
                fullWidth={false}
                loading={busyId === p.id}
                onPress={() => act(p.id, () => superProviders.setTier(p.id, p.tier === "FEATURED" ? "STANDARD" : "FEATURED"))}
              />
            </View>
          </Card>
        ))
    ...
    <View style={{ gap: theme.space(2), backgroundColor: theme.color.surfaceAlt, borderRadius: theme.radius.sm, padding: theme.space(3) }}>
      {q.data!.map((d) => (
        <View key={d.id} style={{ gap: theme.space(1.5) }}>
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
            <AppText size="sm" weight="600">{d.docType.replace(/_/g, " ")}</AppText>
            <Pill
              text={d.status}
